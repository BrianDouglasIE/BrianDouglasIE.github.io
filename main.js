...
for (const entry of await readdir(includeDir)) {
  ...
  includes[entry.replace(".html", "")] = compileTemplate(
    await readFile(join(includeDir, entry)),
    "data",
  );
}
...
for (const entry of await readdir(viewDir)) {
  ...
  views[entry.replace(".html", "")] = compileTemplate(
    await readFile(join(viewDir, entry), "utf8"),
    "data",
  );
}
...
for (const entry of await readdir(postsDir)) {
  ...
  postWriteQueue.push(
    writeFile(join(targetDir, "index.html"), view("post", post)),
  );
  if (!existsSync(getOgImagePath(post)))
...
await writeFile(
  ...
  view("index", { posts: postsDateDesc }),
);
...
async function generateOgImage(post) {
  try {
    ...
    const svgText = createWrappedSvg(
      post.data.title,
      metadata.width,
      metadata.height,
    );
